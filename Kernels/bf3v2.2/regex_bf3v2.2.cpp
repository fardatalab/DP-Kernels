...
{
    // set info in the catalogue
    catalogue->line_rate = (size_t)4000 * 1024 * 1024; // 4000 MB/s

    catalogue->coalescing_enabled = true;

    bf3v2_2_regex_state.global_doca_state = &dpm_doca_state;

    uint64_t max_size = 0;
    doca_regex_get_maximum_job_size(doca_dev_as_devinfo(bf3v2_2_regex_state.global_doca_state->state.dev), &max_size);
    printf("regex max single job size: %lu\n", max_size);
    catalogue->max_single_task_size = max_size;
