...
/// @param result the result of the registration, should fill in the cpu mask
bool bf3_decompress_deflate_kernel_init(struct dpm_kernel_catalogue *result);

bool bf3_decompress_deflate_kernel_cleanup();

bool bf3_decompress_deflate_handle_mem_req(struct dpm_mem_req *req);

uint32_t bf3_decompress_deflate_kernel_can_execute_kernels(int thread_id);

dpkernel_error bf3_decompress_deflate_kernel_execute(dpkernel_task_base *task, int thread_id);
