...
extern "C"
{
                                                  ...
                                                  struct doca_dev **retval)
    {
        ...
        *retval = NULL;
        ...
        {
            printf("Value size too large. Failed to locate device");
    ...
    doca_error_t open_doca_device_with_capabilities(tasks_check func, struct doca_dev **retval)
    {
        ...
        *retval = NULL;
        ...
        {
            printf("Failed to load doca devices list. Doca_error value: %d", result);
            return result;
        }
                                                ...
                                                const uint8_t *value, size_t val_size, struct doca_dev_rep **retval)
    {
        ...
        *retval = NULL;
        ...
        {
            printf("Value size too large. Ignored");
            return DOCA_ERROR_INVALID_VALUE;
        }
        memcpy(val_copy, value, val_size);
        ...
        if (result != DOCA_SUCCESS)
        {
            printf("Failed to create devinfo representor list. Representor devices are available only on DPU, do not "
                   "run on Host");
            return DOCA_ERROR_INVALID_VALUE;
        }
                                               ...
                                               const char *pci_addr, struct doca_dev_rep **retval)
    {
        ...
        *retval = NULL;
        ...
        if (result != DOCA_SUCCESS)
        {
            printf("Failed to create devinfo representors list. Representor devices are available only on DPU, do "
                   "not run on Host");
            return DOCA_ERROR_INVALID_VALUE;
        }
    ...
    doca_error_t create_core_objects(struct program_core_objects *state, uint32_t max_bufs)
    {
        ...
        if (res != DOCA_SUCCESS)
        {
            printf("Unable to create source mmap: %s", doca_error_get_descr(res));
            return res;
        }
        res = doca_mmap_add_dev(state->src_mmap, state->dev);
        if (res != DOCA_SUCCESS)
        {
            printf("Unable to add device to source mmap: %s", doca_error_get_descr(res));
        ...
        if (res != DOCA_SUCCESS)
        {
            printf("Unable to create destination mmap: %s", doca_error_get_descr(res));
            goto destroy_src_mmap;
        }
        res = doca_mmap_add_dev(state->dst_mmap, state->dev);
        if (res != DOCA_SUCCESS)
        {
            printf("Unable to add device to destination mmap: %s", doca_error_get_descr(res));
        ...
        if (max_bufs != 0)
        {
            ...
            if (res != DOCA_SUCCESS)
            {
                printf("Unable to start buffer inventory: %s", doca_error_get_descr(res));
                goto destroy_buf_inv;
            }
        }

        // no longer global, kernel would do this per thread on init
        /* res = doca_pe_create(&state->pe);
        if (res != DOCA_SUCCESS)
        {
            printf("Unable to create progress engine: %s", doca_error_get_descr(res));
            goto destroy_buf_inv;
        } */
    ...
    doca_error_t request_stop_ctx(struct doca_pe *pe, struct doca_ctx *ctx)
    {
        ...
        if (tmp_result == DOCA_ERROR_IN_PROGRESS)
        {
            ...
            do
            {
                ...
                if (tmp_result != DOCA_SUCCESS)
                {
                    ...
                    printf("Failed to get state from ctx: %s", doca_error_get_descr(tmp_result));
                    break;
                }
        ...
        {
            DOCA_ERROR_PROPAGATE(result, tmp_result);
            printf("Failed to stop ctx: %s", doca_error_get_descr(tmp_result));
        }
    ...
    doca_error_t destroy_core_objects(struct program_core_objects *state)
    {
        ...
        // no longer global, kernel would do this per thread on cleanup
        /* if (state->pe != NULL)
        {
            ...
            {
                DOCA_ERROR_PROPAGATE(result, tmp_result);
                printf("Failed to destroy pe: %s", doca_error_get_descr(tmp_result));
            }
            state->pe = NULL;
        } */
        ...
        if (state->buf_inv != NULL)
        {
            ...
            {
                DOCA_ERROR_PROPAGATE(result, tmp_result);
                printf("Failed to destroy buf inventory: %s", doca_error_get_descr(tmp_result));
            }
            state->buf_inv = NULL;
        }
        ...
        if (state->dst_mmap != NULL)
        {
            ...
            {
                DOCA_ERROR_PROPAGATE(result, tmp_result);
                printf("Failed to destroy destination mmap: %s", doca_error_get_descr(tmp_result));
            }
            state->dst_mmap = NULL;
        }
        ...
        if (state->src_mmap != NULL)
        {
            ...
            {
                DOCA_ERROR_PROPAGATE(result, tmp_result);
                printf("Failed to destroy source mmap: %s", doca_error_get_descr(tmp_result));
            }
            state->src_mmap = NULL;
        }
        ...
        if (state->dev != NULL)
        {
            ...
            {
                DOCA_ERROR_PROPAGATE(result, tmp_result);
                printf("Failed to close device: %s", doca_error_get_descr(tmp_result));
            }
            state->dev = NULL;
        }
