...
extern "C"
{
    ...
    doca_error_t open_doca_device_with_pci(const char *pci_addr, jobs_check func, struct doca_dev **retval)
    {
        ...
        *retval = NULL;
        ...
        for (i = 0; i < nb_devs; i++)
        {
            ...
            if (res == DOCA_SUCCESS && buf.raw == bdf.raw)
            {
                                                ...
                                                const uint8_t *value, size_t val_size, struct doca_dev_rep **retval)
    {
        ...
        *retval = NULL;
        ...
        {
            DOCA_LOG_ERR(
                "Failed to create devinfo representor list. Representor devices are available only on DPU, do not "
                "run on Host");
            return DOCA_ERROR_INVALID_VALUE;
        }
    ...
    //     {
    //         DOCA_LOG_ERR(
    //             "Failed to create devinfo representors list. Representor devices are available only on DPU, do "
    //             "not run on Host");
    //         return DOCA_ERROR_INVALID_VALUE;
    //     }
    ...
    /* doca_error_t init_core_objects_mem(struct program_core_objects *state, uint32_t max_bufs)
    {
        doca_error_t res;
    ...
    /* doca_error_t init_core_objects_workq(struct program_core_objects *state, uint32_t workq_depth)
    {
        doca_error_t res;
    ...
    doca_error_t create_core_objects(struct program_core_objects *state, uint32_t max_bufs)
    {
        ...
        // printf("destination mmap created\n");
        ...
        // printf("buffer inventory created\n");
        ...
        // printf("buffer inventory started\n");

        // need to start mmap first
        res = doca_mmap_start(state->src_mmap);
        if (res != DOCA_SUCCESS)
        {
            printf("doca_mmap_start failed for src_mmap\n");
            return false;
        }
        res = doca_mmap_start(state->dst_mmap);
        if (res != DOCA_SUCCESS)
        {
            printf("doca_mmap_start failed for dst_mmap\n");
            return false;
        }
        printf("doca_mmap_start succeeded for src_mmap and dst_mmap\n");
        ...
        //     return res;
        // }
        // printf("device added to context\n");
        return res;
    }
