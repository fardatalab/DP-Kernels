...
uint32_t bf3v2_2_decompress_deflate_hw_kernel_remaining_capacity(int thread_id, uint32_t *max_capacity)
{
    ...
    {
        *max_capacity = BF3v2_2_DECOMPRESS_DEFLATE_WORKQ_DEPTH + DPM_HW_KERNEL_QUEUE_SIZE;
    }
...
{
    // throughput for 2 threads
    switch (input_size)
    {
    case 51: { // 64B output
        return 2 * (float)input_size / (226.3075622 * MB) * NANOSECONDS_PER_SECOND;
    }
    case 531: { // 1KB output
        return 2 * (float)input_size / (2386.362561 * MB) * NANOSECONDS_PER_SECOND;
    }
    case 1647: { // 4KB output
        return 2 * (float)input_size / (7351.680786 * MB) * NANOSECONDS_PER_SECOND;
    }
    default: {
        printf("bf3v2_2_decompress_deflate no histogram for input size: %d, assuming full speed\n", input_size);
        return (float)input_size / (7351.680786 * MB) * NANOSECONDS_PER_SECOND;
    }
    }
}
...
bool bf3v2_2_decompress_deflate_get_catalogue(dpm_kernel_catalogue *catalogue)
{
    ...
    catalogue->kernel_capacity = BF3v2_2_DECOMPRESS_DEFLATE_WORKQ_DEPTH; // 1 queue per cpu
    return true;
}
...
bool bf3v2_2_decompress_deflate_kernel_init(struct dpm_kernel_catalogue *catalogue, int n_threads)
{
    ...
    // catalogue->max_single_task_size = 2 * GB;
    // catalogue->get_estimated_processing_time_ns = __get_estimated_processing_time;
    ...
    for (int i = 0; i < n_cpus; i++)
    {
        ...
        // XXX: changing semantics to be asic queue + hw kernele queue depth
        kernel_queue_remaining_capacity[i] =
            BF3v2_2_DECOMPRESS_DEFLATE_WORKQ_DEPTH + DPM_HW_KERNEL_QUEUE_SIZE; // 1 queue per cpu
    }
...
dpkernel_error bf3v2_2_decompress_deflate_kernel_poll(dpkernel_task **task, int thread_id)
{
    ...
    else if (ret == DOCA_SUCCESS)
    {
        ...
        *task = (dpkernel_task *)event.user_data.ptr; // we store the task in user_data during execute
        ...
        /* if (*task == NULL)
        {
            printf("bf3v2_2_decompress_deflate_kernel_poll got NULL task from event user_data\n");
        } */
        ...
        // reset the data length so it may be reused (submit again later)
        doca_buf_reset_data_len(dpk_task->dst_doca_buf);
        ...
        char *in_ptr = dpm_get_input_ptr_from_shmptr(dpk_task->in);
        char *out_cpy_ptr = dpm_get_output_ptr_from_shmptr(dpk_task->out_cpy);
        char *in_cpy_ptr = dpm_get_input_ptr_from_shmptr(dpk_task->in_cpy);
        memcpy(in_cpy_ptr, in_ptr, dpk_task->in_size);
        memcpy(out_cpy_ptr, out_ptr, dpk_task->out_size);
#endif
        return DPK_SUCCESS;
    }
    else
    {
        printf("Failed to retrieve workq progress: %s\n", doca_get_error_string(ret));
        // got a completion
        *task = (dpkernel_task *)event.user_data.ptr; // we store the task in user_data during execute

        printf("task from event user_data: %zu\n", dpm_get_shmptr_for_task_ptr(*task));
        (*task)->base.completion.store(DPK_ERROR_FAILED);
        return DPK_ERROR_FAILED;
    }
}
