...
bool dpm_detect_platform()
{
    ...
    if (so_files.empty())
    {
        std::cerr << "No platform specific libraries found in " << PLATFORM_SPECIFIC_LIB_PATH << '\n';
        return false;
    } */
    ...
    // for (const auto &so_file : so_files)
    {
        ...
        detect_platform_func detector = dpm_detect_platform;
        ...
        if (detector(&dpm_underlying_device))
        {
            ...
            register_executors_func register_executors = dpm_register_executors;
            if (!register_executors(dpm_executors))
            {
                printf("Cannot register executors\n");
            }
            printf("executors registered for device %d\n", dpm_underlying_device);
...
dpkernel_error _handle_mem_req(struct dpm_mem_req *req)
{
    ...
    case DPM_MEM_REQ_ALLOC_INPUT: {
        // if there is something originally there, assume that it comes from some other allocation, don't touch it
        if (dpk_task.in == (shm_ptr)0) [[likely]]
        {
        ...
        // else assume it is already allocated, only need to perform kernel specific mem req handling
#ifdef MEMCPY_KERNEL
    ...
    case DPM_MEM_REQ_ALLOC_OUTPUT: {
        // if there is something originally there, assume that it comes from some other allocation, don't touch it
        if (dpk_task.out == (shm_ptr)0) [[likely]]
        {
    ...
    case DPM_MEM_REQ_FREE_INPUT: {
        // if the shm_ptr is 0, assume the allocation comes from some other allocation, don't free it here, app would
        // probably free that other allocation separately
        if (dpk_task.in != (shm_ptr)0) [[likely]]
        {
