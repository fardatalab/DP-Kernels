...
bool _setup_shared_memory(size_t shm_size, struct dpm_shared_mem *dpm_shared_mem, const char *shm_name)
{
    ...
    // also madvise the memory
    // fails on old kernel of elba
...
bool setup_mimalloc(struct dpm_io_mem_region *io_region, struct dpm_shared_mem *req_ctx_shm)
{
    ...
    printf("setup_shared_memory_regions done\n");
    ...
    printf("setup_mi_options done\n");

    // memset the underlying memory to 0
    printf("input shm_ptr: %p, size: %lu\n", io_region->input_region.shm_ptr, io_region->input_region.shm_size);
    printf("output shm_ptr: %p, size: %lu\n", io_region->output_region.shm_ptr, io_region->output_region.shm_size);
    printf("req_ctx shm_ptr: %p, size: %lu\n", req_ctx_shm->shm_ptr, req_ctx_shm->shm_size);
