...
int main() {
    ...
    /// @note uncomment this part when evaluating duckdb baseline implementation
    // ensure the server has finished loading strings into memory
    ...
    //     printf("error: wrong checksum bytes received from server\n");
    //     close(server_sock);
    //     exit(1);
    // }
    ...
    if (nbytes_sent < 0) {
        printf("error: failed to write to server socket %d\n", server_sock);
        close(server_sock);
        exit(1);
    }
    if (nbytes_sent == 0) {
...
ssize_t client_fetch_result(int server_sock, size_t* result_ptr) {
    return read(server_sock, result_ptr, sizeof(size_t));
}