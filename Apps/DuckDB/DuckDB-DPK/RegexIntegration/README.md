...
## Overview
This directory was previously located at `Apps/DuckDB/`; it now lives under
`Apps/DuckDB/DuckDB-DPK/RegexIntegration/` so the DuckDB-facing applications
...
## Setup
* The previous `NetworkEngine` submodule is included here as source for the availability-review snapshot.
...
* Otherwise, run `build/client` first, and then
	...
	* Run `network_engine` under `DP-Kernels/build/Apps/DuckDB/DuckDB-DPK/RegexIntegration/build` on the DPU with `sudo` access and the network configuration file `NetworkEngine/DDSPipeline.json` to run the network engine, which will eventually drive one of `re2_regex_app` and `bf3_regex_app`.
