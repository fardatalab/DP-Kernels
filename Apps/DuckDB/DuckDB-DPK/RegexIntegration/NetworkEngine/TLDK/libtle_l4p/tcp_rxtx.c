	...
	uint32_t pid, uint32_t swcsm)
{
	...
	// NOTE: need to set TSO seg size, should probably set it somewhere during port init
	m->tx_offload = _mbuf_tx_offload(dst->l2_len, dst->l3_len, l4, 1448, 0, 0);
	m->ol_flags |= ol_flags;
	...
	if (likely(s->s.type == TLE_V4)) {
		...
		*/
		if (likely(ol_flags & RTE_MBUF_F_TX_TCP_CKSUM))
		{
			l4h->cksum = 0;
			l3h->hdr_checksum = 0;
		}
...
tle_tcp_stream_send(struct tle_stream *ts, struct rte_mbuf *pkt[], uint16_t num)
{
	...
	// printf("send mss %u\n", mss);
	// printf("s->tx.dst.ol_flags = %lx\n", s->tx.dst.ol_flags);

	// NOTE: s->tx.dst.ol_flags differs from this, this is the minimum needed to use hw offload TSO
	////ol_flags = PKT_TX_TCP_SEG | PKT_TX_IPV4 | PKT_TX_IP_CKSUM | PKT_TX_TCP_CKSUM;
	ol_flags = RTE_MBUF_F_TX_TCP_SEG | RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_TCP_CKSUM;
	// printf("send ol_flags %lx\n", ol_flags);
