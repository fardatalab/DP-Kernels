...
#define DBFILE_PATH "/home/ubuntu/dbfiles/sf1.db"
#define IMDB_PATH "/home/ubuntu/dbfiles/sf1"

ssize_t process_request(duckdb_connection conn, int client_sock, bool prof_brkdown_enabled);
...
int main(int argc, char** argv) {
    ...
    if (argc > 2) {
        std::cout << "usage: build/server [prof_brkdown_enabled]\n";
        exit(1);
    } else if (argc == 2) {
    ...
    if (!setup_db_connection(&db, &conn, NULL, IMDB_PATH)) {
        close(client_sock);
        exit(1);
    }
    ...
    nbytes_sent = process_request(conn, client_sock, prof_brkdown_enabled);
    if (nbytes_sent < 0) {
        teardown_db_connection(&db, &conn);
        close(listen_fd);
        exit(1);
    ...
    // if profiling optioin is enabled then also parse the stat file
