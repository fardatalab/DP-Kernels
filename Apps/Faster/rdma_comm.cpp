...
vector<RDMAConnectionResource> server_threads_connections; // for server side connections
...
bool setup_rdma_connections_server(int NumConnections, int PortBase, int BatchSize, int QueueDepth) {
    ...
    server_threads_connections = vector<RDMAConnectionResource>(NumConnections);
...
{
    // NumConnections qp
    client_threads_connections = vector<RDMAConnectionResource>(NumConnections);
...
int rdma_send(RDMAConnectionResource &conn, int slot_index, u_int32_t data_size)
{
    ...
    uint64_t send_buf_address = (uint64_t)get_send_buffer_address((u_int8_t *)conn.send_buf, slot_index, data_size);
...
int rdma_post_all_recv_buffers_in_queue(RDMAConnectionResource &conn, int queue_depth, u_int32_t data_size)
{
    for (uint64_t i = 0; i < queue_depth; ++i)
    {
        ...
        ibv_recv_wr *bad_wr;
        printf("[DEBUG] Posting RECV: wr_id=%lu addr=%p data_size=%u\n", i, sge.addr, data_size);
        ...
        printf("rdma_post_all_recv_buffers_in_queue: posted recv buffer for slot %lu\n", i);
    }
    return 0;
}
...
u_int8_t *rdma_recv(int thread_idx, int slot_index, u_int32_t data_size)
{
    ...
    RDMAConnectionResource &conn = client_threads_connections[thread_idx];
