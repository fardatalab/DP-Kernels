...
double ComputeVariance(double *Measurements, size_t Length, double Mean)
{
    if (Length <= 1)
    {
        return 0;
...
static void GetStatistics(double *Data, size_t DataLength, Statistics *AllStatistics, Percentiles *PercentileStats)
{
    if (DataLength == 0)
    {
        return;
    ...
    double Variance = ComputeVariance(Data, DataLength, Mean);
