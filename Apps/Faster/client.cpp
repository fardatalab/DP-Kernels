                      ...
                      std::chrono::high_resolution_clock::time_point *EndTime, uint64_t *BytesCompleted)
{
    ...
    RDMAConnectionResource &client_conn = client_threads_connections[ThreadNum];

    // Send queue depth number of batches
    uint64_t bytesCompleted = 0; // this is the number of bytes of payload received (uncompressed)
    ...
    *StartTime = high_resolution_clock::now();
    ...
    for (int q = 0; q != QueueDepth; q++)
    {
        ...
        for (int i = 0; i != BatchSize; i++)
        {
            ...
            // hdr = (MessageHeader *)(batch_buf_start + i * MAX_SEND_MSG_SIZE);
            hdr = (MessageHeader *)(batch_buf_start + i * HeaderSize);
            hdr->TimeSend = high_resolution_clock::now().time_since_epoch().count();
            hdr->BatchId = (uint16_t)q;
            hdr->Key = Keys[msgIndex * BatchSize + i];
        ...
        // printf("Thread %d sending key: %lu, batchId: %d\n", ThreadNum, hdr->Key, hdr->BatchId);

        rdma_send_batch(client_conn, q, MAX_SEND_BATCH_SIZE, TOTAL_MESSAGE_SIZE);
        bytesSent += TOTAL_MESSAGE_SIZE;
    }
    ...
    while (!over)
    {
        ...
        for (int i = 0; i != BatchSize; i++)
        {
            ...
            ////hdr = (MessageHeader *)(recv_buf_address + i * HeaderSize);
            hdr = (MessageHeader *)(recv_buf_address + i * MAX_RECV_MSG_SIZE);
            ...
            // there is no guarantee that the BatchID is the same as the recv_wr......
        ...
        u_int8_t *batch_buf_start = get_send_buffer_address(conn.send_buf, BatchID, MAX_SEND_BATCH_SIZE);
        if (msgIndex < n_batches) [[likely]] // send one more batch if we haven't sent all yet
        {
            ...
            /// XXX: wr_id from recv say nothing about the slot we should use for sending, use batch_id instead
            rdma_send_batch(client_conn, BatchID, MAX_SEND_BATCH_SIZE, TOTAL_MESSAGE_SIZE);
            ////rdma_send_batch(client_conn, recv_wr, MAX_SEND_BATCH_SIZE, TOTAL_MESSAGE_SIZE);
        ...
        rdma_post_one_recv_buffer(client_conn, MAX_RECV_BATCH_SIZE, recv_wr);

        bytesCompleted += MAX_RECV_BATCH_SIZE; // we received a full batch of responses
...
int main(int argc, char *argv[])
{
    ...
    client_threads_connections.resize(N_THREADS);
    setup_rdma_connections_client(N_THREADS, DPU_PORT_BASE, QUEUE_DEPTH, BATCH_SIZE, max_send_msg_size,
                                  max_recv_msg_size, client_threads_connections);
    ...
    double throughput = (double)aggBytesCompleted / (double)duration * 1000000.0f / (1e6); // in MB per second
    ...
    printf("IOPS: %d RPS\n", static_cast<int>(per_thread_read_num * N_THREADS / ((double)duration / 1000000)));
