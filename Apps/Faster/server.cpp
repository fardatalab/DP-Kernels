...
void dump_compressed_value(const CompressedValue* val) {
    const uint8_t* data = reinterpret_cast<const uint8_t*>(val);
    size_t size = sizeof(CompressedValue);

    printf("CompressedValue (%zu bytes):\n", size);
                          ...
                          int QueueDepth, uint64_t ReadNum)
{
    ...
    while (!over)
    {
        ...
        printf("\nThread %zu received wr_id: %lu\n", thread_idx, wr_id);

        // cout << "Thread " << thread_idx << "received " << tcpNum + 1 << "message" << endl;
        MessageHeader *requestMsg;
        // u_int8_t *recv_buf = get_recv_buffer_address(conn.recv_buf, wr_id, MAX_RECV_MSG_SIZE);
        u_int8_t *recv_buf = (uint8_t*) (conn.recv_buf + wr_id * MAX_RECV_BATCH_SIZE); 
        ...
        for (int i = 0; i < BatchSize; i++)
        {
            ...
            case OP::READ: {
                ...
                printf("key: %lu, batchid: %lu\n", requestMsg->Key, requestMsg->BatchId);
                ...
                CompressedValue* val = context.GetValue();
                if ((void*)val != (void*)compressed_buf) {
                    printf("copying...\n");
                    memcpy(compressed_buf, val, sizeof(CompressedValue));
                }

                // decompress the value and put it into the send buffer
                size_t actual_out_size = 0;

                // dump_compressed_value((CompressedValue*)compressed_buf);   
                printf("size of compressedvalue: %zu\n", sizeof(CompressedValue));
                printf("sendbuf: %p, conn.send_buf: %p, diff: %zu\n", send_buf, conn.send_buf, send_buf - (u_int8_t*)conn.send_buf);

                printf("send_buf: %p, MessageHeader: %lu, UncompressedValue: %lu\n",
                ...
                if (deflate_decompress_data(thread_idx, compressed_buf, sizeof(CompressedValue),
                                            send_buf + sizeof(MessageHeader), sizeof(UncompressedValue),
                                            &actual_out_size) == 0)
                {
            ...
            case OP::UPSERT: {
                ...
                u_int8_t *recv_buf_batch_base = get_recv_buffer_address(conn.recv_buf, wr_id, MAX_RECV_MSG_SIZE);
        ...
        printf("Posting wr_id: %lu\n", wr_id);
        
        // rearm the slot for receive
        rdma_post_one_recv_buffer(conn, MAX_RECV_BATCH_SIZE, wr_id);

        // now send the batch of responses, with corresponding size
        rdma_send(conn, wr_id, this_resp_size * BatchSize);
    ...
    cout << "dur: " << (double)dur.count() / 1000000000 << endl;
...
int main(int argc, char *argv[])
{
    ...
    init_libdeflate(N_THREADS);

    // 1. Allocate and fill garbage
    uint8_t* file_buffer = (uint8_t*)malloc(1647);
    uint8_t* uncompressed_data = (uint8_t*)malloc(4096);
    for (size_t i = 0; i < 4096; ++i)
        uncompressed_data[i] = 0x11;

    int buffer_size = deflate_compress_data(0, uncompressed_data, 4096, file_buffer, 1647);
    printf("Read %zu bytes from input file: %s\n", buffer_size, INPUT_FILE);
    dump_compressed_value((const CompressedValue*)file_buffer);
