                                 ...
                                 int QueueDepth, uint64_t ReadNum)
{
    ...
    const int MAX_SEND_MSG_SIZE =
        (sizeof(MessageHeader) + sizeof(CompressedValue)); // only sending out compressed values
    const int MAX_RECV_BATCH_SIZE = BatchSize * MAX_RECV_MSG_SIZE;
    const int MAX_SEND_BATCH_SIZE = BatchSize * MAX_SEND_MSG_SIZE;
    // const int MAX_READ_RECV_MSG_SIZE = sizeof(MessageHeader); // read OP response
    // const int MAX_READ_SEND_MSG_SIZE = (sizeof(MessageHeader) + sizeof(CompressedValue));
    ...
    while (!over)
    {
        ...
        for (int i = 0; i < BatchSize; i++)
        {
            ...
            case OP::READ: {
                ...
                // put the read value directly into the send buffer
                memcpy(send_buf + sizeof(MessageHeader), read_value, sizeof(CompressedValue));
                // copy the header to the send buffer
                memcpy(send_buf, requestMsg, sizeof(MessageHeader));
...
int main(int argc, char *argv[])
{
    ...
    server_threads_connections.resize(N_THREADS);
    setup_rdma_connections_server(N_THREADS, PORT_BASE, BATCH_SIZE, QUEUE_DEPTH, server_threads_connections);
