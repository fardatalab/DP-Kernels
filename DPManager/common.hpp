...
#define TEST_USE_MEMCPY        // pass not the pointers to payload buffers, but copy the payload themselves
#define TEST_PAYLOAD_SIZE 4096 // 4KB
#undef TEST_USE_MEMCPY

// #define SHM_NAME_SUBMISSION_COMPLETION "dpm_submission_completion"
...
/// we pass pointers in shared memory as an offset from the start of the shared memory region
...
enum dpkernel_name
{
    ...
    NULL_KERNEL,
    KERNEL_NAME_LAST // keep last don't use
};

// supported devices
// e.g. application can select some devices, initialize them,
// and the dp kernel manager will schedule accordingly
enum dpkernel_device
{
    DEVICE_BLUEFIELD_2,
    DEVICE_BLUEFIELD_3,
    DEVICE_SOFTWARE,
    DEVICE_NULL // keep last don't use
};

// NOTE: the application no longer specify a performance hint
// it should just choose where (which dpkernel_device) to execute the compute kernel on
/* enum perf_hint
{
    PERF_HINT_LATENCY,
...
enum dpkernel_error
{
    ...
    DPK_ERROR    // task failed (consider it as completed)
};

/// this is the base struct for all tasks that the application can submit to the DPManager.
/// Essentially, this is the interface.
/// DP Manager will construct the corresponding task struct for execution
...
struct dpkernel_task_base
{
    ...
    /// some context that the user may or may not need, pass in upon task submission, get the same thing back
...
/// the completion struct that the application can get back from the DPManager after submitting and executing a task
/// e.g. the null executor holds a queue of tasks, and also a queue of completions where dpm polls from
#ifdef TEST_USE_MEMCPY
...
/// this lives in shared memory and is accessed concurrently by the manager (consumer) and applications (producers)
/// templated on the task type (e.g. task base, or specialized for DOCA, etc)
