...
void _dpm_thread_handler(void *args)
{
    ...
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(4 + thread_id, &cpuset); // leave first 4 cores for client app
    int rc = pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset);
    if (rc != 0)
    {
        std::cerr << "Error calling pthread_setaffinity_np for thread " << (int)(intptr_t)args
                  << " on core " << (4 + thread_id) << ": " << strerror(rc) << std::endl;
        return;
    }
...
dpkernel_error _dpm_execute_task(dpkernel_task_base *task, int thread_id)
{
    dpkernel_error ret;
    ...
    struct dpm_kernel *executor = get_kernel(task->device, (enum dpm_task_name)task->task);
...
dpkernel_error _handle_mem_req(struct dpm_mem_req *req)
{
    ...
    {
        char *buf = allocate_input_buf(task->in_size);
        if (buf == nullptr)
        {
        ...
        task->in = dpm_get_shm_ptr_for_input_buf(buf);
#ifdef MEMCPY_KERNEL
        // also allocate the cpy buffers
        char *in_cpy_buf = allocate_input_buf(task->in_size);
        if (in_cpy_buf == nullptr)
        {
            printf("failed to allocate input cpy buf, size = %lu\n", task->in_size);
            req->completion.store(DPK_ERROR_FAILED, std::memory_order_release);
            return DPK_ERROR_FAILED;
        }
        task->in_cpy = dpm_get_shm_ptr_for_input_buf(in_cpy_buf);
#endif
    ...
    {
        char *buf = allocate_output_buf(task->out_size);
        if (buf == nullptr)
        {
        ...
        task->out = dpm_get_shm_ptr_for_output_buf(buf);
#ifdef MEMCPY_KERNEL
        // also allocate the cpy buffers
        char *out_cpy_buf = allocate_output_buf(task->out_size);
        if (out_cpy_buf == nullptr)
        {
            printf("failed to allocate output cpy buf, size = %lu\n", task->out_size);
            req->completion.store(DPK_ERROR_FAILED, std::memory_order_release);
            return DPK_ERROR_FAILED;
        }
        task->out_cpy = dpm_get_shm_ptr_for_output_buf(out_cpy_buf);
#endif
    ...
    {
        mi_free(dpm_get_input_ptr_from_shmptr(task->in));
#ifdef MEMCPY_KERNEL
        mi_free(dpm_get_input_ptr_from_shmptr(task->in_cpy));
#endif
        break;
    }
    case DPM_MEM_REQ_FREE_OUTPUT:
    {
        mi_free(dpm_get_output_ptr_from_shmptr(task->out));
#ifdef MEMCPY_KERNEL
        mi_free(dpm_get_output_ptr_from_shmptr(task->out_cpy));
#endif
        break;
    }
    default:
