...
struct RequestRingBufferProgressive *req_ring_buffer = nullptr;
...
{
    req_ring_buffer = AllocateRequestBufferProgressive();
    if (req_ring_buffer == nullptr)
    {
        printf("AllocateRequestBufferProgressive failed\n");
        return false;
    }
    return true;
}
...
struct __attribute__((packed)) req_result
{
    uint32_t req_size;
    shm_ptr req_ptr;
};
...
bool dpm_poll_submission_msgq()
{
    ...
    // struct req_result req_res = {};
    char *req_res = (char *)malloc(sizeof(uint32_t) + sizeof(shm_ptr));
    ...
    if (mq_polled_submission == NULL)
    {
        ...
        if (FetchFromRequestBufferProgressive(req_ring_buffer, req_res, &req_size))
        {
            // *mq_polled_submission = req_res.req_ptr;
            *mq_polled_submission = *(shm_ptr *)(req_res + sizeof(uint32_t));
            print_debug("got shm_ptr = %lu\n", *mq_polled_submission);
            print_debug("req_size = %u\n", req_size);
            // print_debug("req_res.req_size: %u\n", req_res.req_size);
            // print_debug("got submission, req size = %u\n", req_size);
            ret = _dpm_execute_task_msgq();
            if (ret)
            {
                mq_polled_submission = NULL;
