...
struct dpm_kernel_catalogue
{
    ...
    /// the maximum size of a single task that the kernel can handle, this will be useful for coalescing (up to this
    /// size)
...
struct dpm_kernel
{
    ...
    uint32_t (*hw_kernel_remaining_capacity)(int thread_id);
