...
struct dpm_kernel_registration_result
{
    // CPU mask on which the kernel can be run
    cpu_set_t cpu_mask;
};
...
struct dpm_kernel
{
    ...
    bool (*initialize)(struct dpm_kernel_registration_result *result);
    ...
    dpkernel_error (*execute)(dpkernel_task_base *task, int thread_id);
    ...
    dpkernel_error (*poll)(dpkernel_task_base *task, int thread_id);
...
extern struct dpm_kernel_registration_result dpm_kernel_registration_results[dpm_device::DEVICE_LAST]
