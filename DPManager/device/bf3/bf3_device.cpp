...
bool dpm_detect_platform(enum dpm_device *device)
{
    ...
    // Check if version starts with "3" or "2.5"
    if (version[0] == '3' || (version[0] == '2' && version[1] == '.' && version[2] == '5'))
    {
...
bool dpm_register_executors(dpm_kernel global_executors[DEVICE_LAST][TASK_NAME_LAST])
{
    ...
    global_executors[DEVICE_NULL][TASK_NULL] =
    ...
    global_executors[DEVICE_BLUEFIELD_3][TASK_DECOMPRESS_DEFLATE] = (struct dpm_kernel){
        ...
        .get_catalogue = bf3_decompress_deflate_get_catalogue,
        .cleanup = bf3_decompress_deflate_kernel_cleanup,
        .handle_mem_req = bf3_decompress_deflate_handle_mem_req,
        .hw_kernel_remaining_capacity = bf3_decompress_deflate_kernel_can_execute_kernels,
    ...
    // init the bf3 AES-GCM decrypt executor (prototype)
    global_executors[DEVICE_BLUEFIELD_3][TASK_AES_GCM] = (struct dpm_kernel){
        .initialize = bf3_aes_gcm_kernel_init,
        .get_catalogue = bf3_aes_gcm_get_catalogue,
        .cleanup = bf3_aes_gcm_kernel_cleanup,
        .handle_mem_req = bf3_aes_gcm_handle_mem_req,
        .hw_kernel_remaining_capacity = bf3_aes_gcm_kernel_can_execute_kernels,
        .execute = bf3_aes_gcm_kernel_execute,
        .poll = bf3_aes_gcm_kernel_poll,
    };
    printf("bf3 aes_gcm executor registered for device %d, task %d\n", DEVICE_BLUEFIELD_3, TASK_AES_GCM);
    return true;
}
...
bool setup_doca_shared_mem(struct dpm_doca_state *dpm_doca_state, struct dpm_io_mem_region *mem_region)
{
    ...
    if (doca_mmap_set_memrange(dpm_doca_state->state.dst_mmap, dst_mem_ptr, dst_mem_size) != DOCA_SUCCESS)
    {
