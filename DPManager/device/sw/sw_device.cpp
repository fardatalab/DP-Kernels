...
SWKernelQueue **sw_kernel_ring_buffers;
...
bool dpm_init_sw_kernels()
{
    ...
    sw_kernel_ring_buffers = (SWKernelQueue **)calloc(N_SW_KERNELS_THREADS, sizeof(SWKernelQueue *));

    // init the ring buffers
    for (int i = 0; i < N_SW_KERNELS_THREADS; i++)
    {
        sw_kernel_ring_buffers[i] =
...
{
    // pin thread to core
    int base_core = DPK_THREAD_PIN_BASE_CORE + N_DPM_THREADS;
    int target_core = base_core + args->thread_id;
    long online_cores = sysconf(_SC_NPROCESSORS_ONLN);
    if (online_cores > 0 && target_core >= online_cores)
    {
        target_core %= (int)online_cores;
        printf("SW kernel thread %d requested core %d exceeds online cores=%ld; wrapping to core %d\n",
               args->thread_id, base_core + args->thread_id, online_cores, target_core);
    }

    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(target_core, &cpuset); // pin after DPM threads
    int rc = pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset);
    if (rc != 0)
    {
        std::cerr << "Error calling pthread_setaffinity_np for sw kernel thread " << args->thread_id << ": "
                  << strerror(rc) << std::endl;
        std::cerr << "Continuing sw kernel thread " << args->thread_id << " without explicit pinning" << std::endl;
    }
    else
    {
    ...
    SWKernelQueue *ring_buffer = sw_kernel_ring_buffers[thread_id];
    ...
    while (args->run.load(std::memory_order_acquire))
    {
        ...
        {
            if (FetchFromRequestBufferProgressive<SW_KERNELS_QUEUE_SIZE, SW_KERNELS_QUEUE_MAX_TAIL_ADVANCEMENT>(
                    ring_buffer, buf_holder, &req_size))
            {
                ...
                while (offset < req_size)
                {
                    ...
                    print_debug("dpkernel task %p, device %d, task %d\n", dpk_task, dpk_task->base.device,
                                dpk_task->base.task);
#ifdef CHECK_LATENCY
                    dpk_task->base.execution_timestamp =
...
#ifdef CHECK_LATENCY
                            dpk_task->base.completion_timestamp =
                                std::chrono::high_resolution_clock::now().time_since_epoch().count();
#endif
                        }
                        break;
