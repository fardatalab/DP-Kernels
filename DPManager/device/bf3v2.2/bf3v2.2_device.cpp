#include "bf3v2.2_device.hpp"
#include "common.hpp"
#include "decompress_deflate_bf3v2.2.hpp"
#include "doca_error.h"
#include "doca_mmap.h"
#include "null_kernel.hpp"
#if DPK_HAS_DOCA_REGEX
#include "regex_bf3v2.2.hpp"
#endif
#include <cstddef>
#include <cstdio>
#include <cstdlib>
#pragma clang diagnostic ignored "-Wdeprecated-declarations"

struct dpm_doca_state dpm_doca_state;

void *dpm_setup_device_ctx = NULL;

bool dpm_detect_platform(enum dpm_device *device)
{
    *device = DEVICE_BLUEFIELD_3;
    printf("underlying device detected as BlueField-3\n");
    const char *version_file_path = "/opt/mellanox/doca/applications/VERSION";
    FILE *file = fopen(version_file_path, "r");
    if (!file)
    {
        printf("Failed to open DOCA version file\n");
        return false;
    }

    char version[64];
    if (fgets(version, sizeof(version), file) == NULL)
    {
        printf("Failed to read DOCA version\n");
        fclose(file);
        return false;
    }

    fclose(file);

    // Check if version is DOCA v2.2
    if (version[0] == '2' && version[1] == '.' && version[2] == '2')
    {
        printf("Detected BlueField-3 v2.2 platform (DOCA %s)\n", version);
        return true;
    }

    return false;
}

bool dpm_generate_device_ctx()
{
    struct bf3v2_2_device_init_ctx *setup_bf3v2_2_ctx =
        (struct bf3v2_2_device_init_ctx *)malloc(sizeof(struct bf3v2_2_device_init_ctx));
    setup_bf3v2_2_ctx->pci_addr = const_cast<char *>(DPM_DOCA_PCI_ADDR);
    setup_bf3v2_2_ctx->dpm_doca_state = &dpm_doca_state;
    dpm_setup_device_ctx = (void *)setup_bf3v2_2_ctx;
    return true;
}

bool dpm_register_executors(dpm_kernel global_executors[DEVICE_LAST][TASK_NAME_LAST])
{
    // memset(global_executors, 0, sizeof(global_executors));

    // init the null executor
    global_executors[dpm_device::DEVICE_NULL][dpm_task_name::TASK_NULL] =
        (struct dpm_kernel){.initialize = NULL,
                            .get_catalogue = null_kernel_get_catalogue,
                            .hw_kernel_remaining_capacity = null_kernel_can_execute_kernels,
                            .execute = null_kernel_execute,
                            .poll = NULL};
    printf("null executor registered for device %d, task %d\n", DEVICE_NULL, TASK_NULL);

    // init the bf3 executor
    global_executors[DEVICE_BLUEFIELD_3][TASK_DECOMPRESS_DEFLATE] = (struct dpm_kernel){
        .initialize = bf3v2_2_decompress_deflate_kernel_init,
        .get_catalogue = bf3v2_2_decompress_deflate_get_catalogue,
        .cleanup = bf3v2_2_decompress_deflate_kernel_cleanup,
        .handle_mem_req = bf3v2_2_decompress_deflate_handle_mem_req,
        .hw_kernel_remaining_capacity = bf3v2_2_decompress_deflate_hw_kernel_remaining_capacity,
        .execute = bf3v2_2_decompress_deflate_kernel_execute,
        .poll = bf3v2_2_decompress_deflate_kernel_poll,
    };
    printf("bf3v2_2 decompress executor registered for device %d, task %d\n", DEVICE_BLUEFIELD_3,
           TASK_DECOMPRESS_DEFLATE);

#if DPK_HAS_DOCA_REGEX
    global_executors[DEVICE_BLUEFIELD_3][TASK_REGEX_SEARCH] = (struct dpm_kernel){
        .initialize = bf3v2_2_regex_kernel_init,
        .get_catalogue = bf3v2_2_regex_get_catalogue,
        .cleanup = bf3v2_2_regex_kernel_cleanup,
        .handle_mem_req = bf3v2_2_regex_handle_mem_req,
        .hw_kernel_remaining_capacity = bf3v2_2_regex_hw_kernel_remaining_capacity,
        .execute = bf3v2_2_regex_kernel_execute,
        .poll = bf3v2_2_regex_kernel_poll,
    };
    printf("bf3v2_2 regex executor registered for device %d, task %d\n", DEVICE_BLUEFIELD_3, TASK_REGEX_SEARCH);
#else
    printf("bf3v2_2 regex executor skipped (DPK_HAS_DOCA_REGEX=0)\n");
#endif

    return true;
}

void free_mmap_callback(void *addr, size_t len, void *opaque)
{
    // do nothing
}

bool setup_doca_shared_mem(struct dpm_doca_state *dpm_doca_state, struct dpm_io_mem_region *mem_region)
{
    doca_error_t result;
    void *dst_mem_ptr = mem_region->output_region.shm_ptr;
    size_t dst_mem_size = mem_region->output_region.shm_size;
#if DPK_SINGLE_IO_REGION
    // Single-IO mode: source and destination mmaps intentionally point at the same backing region.
    dst_mem_ptr = mem_region->input_region.shm_ptr;
    dst_mem_size = mem_region->input_region.shm_size;
#endif

    // init the spinlock for the single buf inventory
    pthread_spin_init(&dpm_doca_state->buf_inventory_lock, PTHREAD_PROCESS_PRIVATE);

    if ((result = doca_mmap_set_memrange(dpm_doca_state->state.src_mmap, mem_region->input_region.shm_ptr,
                                         mem_region->input_region.shm_size)) != DOCA_SUCCESS)
    {
        printf("doca_mmap_set_memrange failed for src_mmap: %s\n", doca_get_error_string(result));
        return false;
    }
    printf("doca_mmap_set_memrange succeeded for src_mmap\n");
    if ((result = doca_mmap_set_memrange(dpm_doca_state->state.dst_mmap, dst_mem_ptr, dst_mem_size)) != DOCA_SUCCESS)
    {
        printf("doca_mmap_set_memrange failed for dst_mmap %s\n", doca_get_error_string(result));
        return false;
    }
    printf("doca_mmap_set_memrange succeeded for dst_mmap\n");

    result = doca_mmap_start(dpm_doca_state->state.src_mmap);
    if (result != DOCA_SUCCESS)
    {
        printf("doca_mmap_start failed for src_mmap\n");
        return false;
    }
    result = doca_mmap_start(dpm_doca_state->state.dst_mmap);
    if (result != DOCA_SUCCESS)
    {
        printf("doca_mmap_start failed for dst_mmap\n");
        return false;
    }
    print_debug("doca_mmap_start succeeded for src_mmap and dst_mmap\n");

    /* result = doca_mmap_dev_add(dpm_doca_state->state.src_mmap, dpm_doca_state->state.dev);
    if (result != DOCA_SUCCESS)
    {
        printf("Unable to add device to source mmap: %s\n", doca_get_error_string(result));
        doca_mmap_destroy(dpm_doca_state->state.src_mmap);
        dpm_doca_state->state.src_mmap = NULL;
        return result;
    }
    result = doca_mmap_dev_add(dpm_doca_state->state.dst_mmap, dpm_doca_state->state.dev);
    if (result != DOCA_SUCCESS)
    {
        printf("Unable to add device to destination mmap: %s\n", doca_get_error_string(result));
        doca_mmap_destroy(dpm_doca_state->state.dst_mmap);
        dpm_doca_state->state.dst_mmap = NULL;
        return result;
    }
    printf("doca_mmap_dev_add succeeded for src_mmap and dst_mmap\n"); */

    return true;
}

bool dpm_setup_memory_regions(void *ctx, struct dpm_io_mem_region *io_region)
{
    struct bf3v2_2_device_init_ctx *bf2_ctx = (struct bf3v2_2_device_init_ctx *)ctx;
    struct dpm_doca_state *dpm_doca_state = bf2_ctx->dpm_doca_state;

    if (!setup_doca_shared_mem(dpm_doca_state, io_region))
    {
        printf("setup_doca_shared_mem failed\n");
        return false;
    }
    printf("setup_doca_shared_mem succeeded\n");

    return true;
}

bool dpm_setup_device(void *ctx)
{
    bool ret;
    struct bf3v2_2_device_init_ctx *bf2_ctx = (struct bf3v2_2_device_init_ctx *)ctx;
    const char *pci_addr = bf2_ctx->pci_addr;
    struct dpm_doca_state *dpm_doca_state = bf2_ctx->dpm_doca_state;
    // struct dpm_io_mem_region *dpm_own_mem_region = bf3_ctx->dpm_own_mem_region;

    if (open_doca_device_with_pci(pci_addr, NULL, &dpm_doca_state->state.dev) != DOCA_SUCCESS)
    {
        printf("open_doca_device_with_pci failed\n");
        // XXX HACK: return false; // shouldn't fail, not returning for testing purposes
    }
    printf("open_doca_device_with_pci succeeded\n");

    // TODO: This MUST be done by DPM!!
    /* bool ret = setup_mimalloc(dpm_own_mem_region, &dpm_req_ctx_shm);
    if (!ret)
    {
        printf("setup_mimalloc failed\n");
        return false;
    }
    printf("setup_mimalloc succeeded\n"); */

    // #ifdef DOCA_VER_1_5
    //     ret = create_core_objects(&dpm_doca_state->state, DPM_DOCA_WORKQ_DEPTH);
    //     if (ret != DOCA_SUCCESS)
    //     {
    //         printf("init_core_objects_workq failed\n");
    //         return false;
    //     }
    //     printf("init_core_objects_workq succeeded\n");
    // #elif DOCA_VER_2_5
    ret = create_core_objects(&dpm_doca_state->state, DPM_DOCA_MAX_BUF);
    if (ret != DOCA_SUCCESS)
    {
        printf("create_core_objects failed\n");
        return false;
    }
    // #endif
    printf("create_core_objects succeeded\n");

    // ret = setup_doca_shared_mem(pci_addr, dpm_doca_state, dpm_own_mem_region);
    // if (!ret)
    // {
    //     printf("setup_doca_shared_mem failed\n");
    //     return false;
    // }
    // printf("setup_doca_shared_mem succeeded\n");

    return true;
}

bool dpm_cleanup_device(void *ctx)
{
    auto ret = destroy_core_objects(&dpm_doca_state.state);
    if (ret != DOCA_SUCCESS)
    {
        printf("destroy_core_objects failed: %s\n", doca_get_error_string(ret));
        return false;
    }
    /* if (doca_compress_destroy(dpm_doca_state.compress) != DOCA_SUCCESS)
    {
        printf("doca_compress_destroy failed\n");
        return false;
    } */
    return true;
}
