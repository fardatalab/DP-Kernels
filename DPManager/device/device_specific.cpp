...
bool dpm_device_and_mem_init(struct dpm_io_mem_region *dpm_own_mem_region)
{
    ...
    if (dpm_device_initializers.setup_device_ctx == NULL || 
        dpm_device_initializers.setup_device == NULL ||
        dpm_mem_initializers.setup_memory_regions == NULL)
    {
        std::cerr << "No device initializer found...\n";
        return true;
    }
