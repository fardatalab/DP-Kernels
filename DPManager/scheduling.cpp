...
void _inc_hw_queueing_delay_on_submit_or_enqueue(int thread_id, struct dpkernel_task_base *dpk_task)
{
...
void _inc_sw_queueing_delay_on_submit_or_enqueue(int thread_id, struct dpkernel_task_base *dpk_task)
{
    // calc this task's processing time
    long this_task_time = dpm_kernel_catalogues[dpk_task->device][dpk_task->task].get_estimated_processing_time_ns(
        dpk_task->in_size, thread_id);
    ...
    estimated_sw_kernels_queuing_time[thread_id][dpk_task->task]->fetch_add(this_task_time, std::memory_order_relaxed);
    // get a timestamp for this very moment we incremented the queueing delay
...
void _dec_hw_queueing_delay_on_completion(int thread_id, struct dpkernel_task_base *dpk_task)
{
    last_estimated_hw_kernels_timestamps[thread_id][dpk_task->task] =
        std::chrono::high_resolution_clock::now().time_since_epoch().count();
...
void _dec_sw_queueing_delay_on_completion(int thread_id, struct dpkernel_task_base *dpk_task)
{
    last_estimated_sw_kernels_timestamps[thread_id][dpk_task->task] =
        std::chrono::high_resolution_clock::now().time_since_epoch().count();
