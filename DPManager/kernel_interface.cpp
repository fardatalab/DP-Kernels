...
struct dpm_kernel_registration_result dpm_kernel_registration_results[dpm_device::DEVICE_LAST]
                                                                     [dpm_task_name::TASK_NAME_LAST];
// dpm_task_executor dpm_executors[dpm_task_name::TASK_NAME_LAST];
...
// register the executors for BF-2 or BF-3
/* #ifdef DOCA_VER_1_5
void dpm_register_executors()
{
    ...
    dpm_executors[DEVICE_BLUEFIELD_2][TASK_DECOMPRESS_DEFLATE] = (struct dpm_task_executor){
        .initialize = NULL,
    ...
    dpm_executors[DEVICE_BLUEFIELD_2][TASK_REGEX_COUNT] = (struct dpm_task_executor){
        .initialize = NULL,
        .execute = bf2_regex_count_kernel_execute,
        .poll = bf2_regex_count_kernel_poll,
        .get_estimated_completion_time = bf2_regex_count_kernel_get_estimated_completion_time};
    dpm_executors[DEVICE_BLUEFIELD_2][TASK_REGEX_SEARCH] = (struct dpm_task_executor){
        .initialize = NULL,
        .execute = bf2_regex_search_kernel_execute,
        .poll = bf2_regex_search_kernel_poll,
        .get_estimated_completion_time = bf2_regex_search_kernel_get_estimated_completion_time};
}
#elif DOCA_VER_2_5
void dpm_register_executors()
{
...
void dpm_init_kernels()
{
    for (int i = 0; i < dpm_device::DEVICE_LAST; i++)
    {
        for (int j = 0; j < dpm_task_name::TASK_NAME_LAST; j++)
        {
            ...
            {
                struct dpm_kernel_registration_result *result = &dpm_kernel_registration_results[i][j];
                if (dpm_executors[i][j].initialize(result))
                {
                    printf("executor %d %d initialized\n", i, j);
                }
                else
                {
                    printf("executor %d %d failed to initialize\n", i, j);
                }
            }
        }
    }
