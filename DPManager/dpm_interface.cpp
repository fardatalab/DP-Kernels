...
void dpm_submit_task_msgq_multithread(dpkernel_task_base *submission, dpm_device device)
{
    submission->device = device;
    // convert to shm_ptr
    shm_ptr task_ptr = get_shm_ptr_for_task(submission);
...
bool _dpm_alloc_shared_mem(size_t size, dpkernel_task_base *task, enum dpm_mem_req_type type)
{
...
bool dpm_alloc_input_buf(size_t size, dpkernel_task_base *task)
{
    return _dpm_alloc_shared_mem(size, task, DPM_MEM_REQ_ALLOC_INPUT);
}

bool dpm_alloc_output_buf(size_t size, dpkernel_task_base *task)
{
...
bool _dpm_free_shared_mem(dpkernel_task_base *task, bool is_input)
{
...
bool dpm_free_input_buf(dpkernel_task_base *task)
{
    return _dpm_free_shared_mem(task, true);
}

bool dpm_free_output_buf(dpkernel_task_base *task)
{
