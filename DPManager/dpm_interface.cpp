...
MPMCQueue<shm_ptr> *submission_queue_mpmc;

struct RequestRingBufferProgressive *submission_ring_buffer = nullptr;
...
bool dpm_submit_task_msgq(dpkernel_task_base *submission)
{
    ...
    // XXX: submit with diff queues
    ////return submission_queue->try_send(&task_ptr, sizeof(task_ptr), 0);
    ////return submission_queue_mpmc->try_push(task_ptr);
    return InsertToRequestBufferProgressive(submission_ring_buffer, (char *)&task_ptr, sizeof(shm_ptr));
}
...
bool _dpm_setup_msgq()
{
    ...
    // setup the ring buffer for requests
    submission_ring_buffer = SetupRequestBufferProgressive();
    if (submission_ring_buffer == nullptr)
    {
        printf("SetupRequestBufferProgressive failed\n");
        return false;
    }
    ...
    BOOST_TRY
    {
        ...
        mem_req_queue = new boost::interprocess::message_queue(boost::interprocess::open_only, DPM_MEM_REQ_QUEUE_NAME);
    }
...
{
    return completion_queue->get_num_msg();
}
...
bool _dpm_alloc_shared_mem(size_t size, shm_ptr *buf, enum dpm_mem_req_type type)
{
    ...
    shm_ptr req_ptr = get_shm_ptr_for_mem_req_ctx((char *)req);
...
bool _dpm_free_shared_mem(char *buf, bool is_input)
{
    ...
    struct dpm_mem_req *req =
        (struct dpm_mem_req *)mi_heap_malloc(app_req_ctx_shm.allocator.heap, sizeof(struct dpm_mem_req));
    if (req == NULL)
    {
