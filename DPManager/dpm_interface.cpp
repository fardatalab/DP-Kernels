...
std::vector<MPMCQueue<struct dpm_req_msg, DPM_SUBMISSION_QUEUE_SIZE> *> submission_queues_mpmc;
...
#else
    while (!submission_queue_mpmc->try_push(msg))
    {
        std::this_thread::yield();
    }
    return;
...
#else
    auto submission_queue_mpmc = submission_queues_mpmc[thread_id % MPMC_QUEUE_NUMBER];
    while (!submission_queue_mpmc->try_push(msg))
    {
        std::this_thread::yield();
    }
...
//     }
// #else
//     while (!InsertToRequestBufferProgressive(thread_id, submission_queue_ring_buffer, (char *)&msg, sizeof(msg)))
//         ;
// #endif
// }
...
bool _dpm_setup_msgq()
{
    ...
    {
        std::string mpmc_queue_name = SUBMISSION_QUEUE_NAME + std::to_string(i);
        auto *mpmc_queue = rigtorp::connect_mpmc_queue<dpm_req_msg, DPM_SUBMISSION_QUEUE_SIZE>(mpmc_queue_name.c_str());
        if (mpmc_queue == nullptr)
        {
            printf("setup submission queue ring buffer failed\n");
            return false;
        }
        submission_queues_mpmc.push_back(mpmc_queue);
    }
    ////
