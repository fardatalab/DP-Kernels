...
bool _dpm_alloc_shared_mem(size_t size, dpkernel_task *task, enum dpm_mem_req_type type)
{
...
bool dpm_alloc_input_buf(size_t size, dpkernel_task *task)
{
    return _dpm_alloc_shared_mem(size, task, DPM_MEM_REQ_ALLOC_INPUT);
}

bool dpm_alloc_output_buf(size_t size, dpkernel_task *task)
{
    return _dpm_alloc_shared_mem(size, task, DPM_MEM_REQ_ALLOC_OUTPUT);
}
...
bool app_alloc_task_request(dpkernel_task_bf3 **task)
{
    {
        std::lock_guard<std::mutex> lock(heap_alloc_mutex);
        *task = (dpkernel_task_bf3 *)mi_heap_malloc(app_req_ctx_shm.allocator.heap, sizeof(dpkernel_task_bf3));
    }
    if (task == NULL)
    {
        printf("mi_heap_malloc task failed\n");
...
bool app_alloc_task_request(dpkernel_task_bf2 **task)
{
    {
        std::lock_guard<std::mutex> lock(heap_alloc_mutex);
        *task = (dpkernel_task_bf2 *)mi_heap_malloc(app_req_ctx_shm.allocator.heap, sizeof(dpkernel_task_bf2));
    }
    if (task == NULL)
    {
        printf("mi_heap_malloc task failed\n");
...
bool app_alloc_task_request(dpkernel_task_bf3v2_2 **task)
{
    {
        std::lock_guard<std::mutex> lock(heap_alloc_mutex);
        *task = (dpkernel_task_bf3v2_2 *)mi_heap_malloc(app_req_ctx_shm.allocator.heap, sizeof(dpkernel_task_bf3v2_2));
    }
    if (task == NULL)
    {
        printf("mi_heap_malloc task failed\n");
...
{
    return _dpm_free_shared_mem(task, true);
}

bool dpm_free_output_buf(dpkernel_task *task)
{
    return _dpm_free_shared_mem(task, false);
}
