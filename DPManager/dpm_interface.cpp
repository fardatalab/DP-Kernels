...
/// Ensure this thread has a mimalloc heap bound to app req_ctx shared-memory arena.
/// All app-side req_ctx allocations (tasks, mem requests, metadata buffers) must use this.
static bool _ensure_req_ctx_thread_heap()
{
    ...
    if (heap == NULL)
    {
        printf("mi_heap_new_in_arena failed for req_ctx region thread id = %lu\n", pthread_self());
        return false;
    }
                                  ...
                                  dpm_device device = dpm_device::DEVICE_NONE)
{
    ...
    for (int i = 0; i < n_tasks; i++)
    {
        ...
        submission->device = device;
        // convert to shm_ptr
        shm_ptr task_ptr = get_shm_ptr_for_task(tasks[i]);
        submission->completion.store(DPK_ONGOING);
        msgs[i].type = DPM_REQ_TYPE_TASK;
        msgs[i].ptr = task_ptr;
...
bool dpm_frontend_initialize_inproc()
{
    // Embedded mode reuses manager-owned mappings in-process.
    app_own_mem_region = dpm_own_mem_region;
    app_req_ctx_shm = dpm_req_ctx_shm;

    if (app_req_ctx_shm.shm_ptr == nullptr || app_req_ctx_shm.shm_size == 0)
    {
        printf("dpm_frontend_initialize_inproc got invalid req_ctx mapping\n");
        return false;
    }
    ...
    if (!mi_manage_os_memory_ex(app_req_ctx_shm.shm_ptr, app_req_ctx_shm.shm_size, true, false, false, -1, true,
                                &app_req_ctx_shm.arena_id))
    {
        printf("mi_manage_os_memory_ex failed for req_ctx region in embedded mode\n");
        return false;
    }
...
struct dpm_mem_req *_dpm_submit_shared_mem_req_async_on_thread(dpkernel_task *task, enum dpm_mem_req_type type,
                                                                uint32_t thread_id)
{
    struct dpm_mem_req *req;
    if (!_ensure_req_ctx_thread_heap())
    {
        return nullptr;
    }
    {
...
{
    // Keep legacy behavior: memory requests default to queue 0 when no explicit thread is provided.
    return _dpm_submit_shared_mem_req_async_on_thread(task, type, 0);
}
...
struct dpm_mem_req *dpm_alloc_mem_buf_async(uint32_t in_size, uint32_t out_size, dpkernel_task *task)
{
    task->base.in_size = in_size;
    task->base.out_size = out_size;
    return _dpm_submit_shared_mem_req_async(task, DPM_MEM_REQ_ALLOC_IO);
}

struct dpm_mem_req *dpm_alloc_mem_buf_async_on_thread(uint32_t in_size, uint32_t out_size, dpkernel_task *task,
                                                       uint32_t thread_id)
{
    task->base.in_size = in_size;
    task->base.out_size = out_size;
    return _dpm_submit_shared_mem_req_async_on_thread(task, DPM_MEM_REQ_ALLOC_IO, thread_id);
}

bool dpm_alloc_mem_buf(uint32_t in_size, uint32_t out_size, dpkernel_task *task)
{
    task->base.in_size = in_size;
    task->base.out_size = out_size;
    return _dpm_alloc_shared_mem(task, DPM_MEM_REQ_ALLOC_IO);
}

bool dpm_alloc_input_output_buf(uint32_t in_size, uint32_t out_size, dpkernel_task *task)
{
    // Backward-compatible wrapper for the historical dual-size API.
    return dpm_alloc_mem_buf(in_size, out_size, task);
}
...
bool app_alloc_task_request(dpkernel_task **task)
{
    ...
    (*task)->base.use_lazy_doca_buf_binding = false;
    (*task)->base.io_is_externally_owned = false;
    (*task)->base.in_binding_state = DPM_IO_BINDING_UNBOUND;
    (*task)->base.out_binding_state = DPM_IO_BINDING_UNBOUND;
    (*task)->base.device = dpm_device::DEVICE_LAST;
    // Default: no kernel memory resources bound yet (will be set by DPM on first ALLOC_* mem request).
    (*task)->base.mem_device = dpm_device::DEVICE_NONE;
    (*task)->base.task = dpm_task_name::TASK_NAME_LAST;
    (*task)->base.completion.store(DPK_ONGOING);
    (*task)->base.is_coalesced_head = false;

    // Defensive: task unions are allocated via mi_heap_malloc (no constructor), so make sure any
    // device-specific fields that might be freed later start in a known state.
    (*task)->bf3.src_doca_buf = NULL;
    (*task)->bf3.dst_doca_buf = NULL;
    (*task)->bf3.prealloc_decompress_task = NULL;
    (*task)->bf3.prealloc_thread_id = UINT8_MAX;
    (*task)->bf3.callback_status = DPK_ONGOING;
    (*task)->bf3.callback_owner_thread_id = 0;
    (*task)->bf3_aes_gcm.src_doca_buf = NULL;
    (*task)->bf3_aes_gcm.dst_doca_buf = NULL;
    (*task)->bf3_aes_gcm.prealloc_decrypt_task = NULL;
    (*task)->bf3_aes_gcm.prealloc_thread_id = UINT8_MAX;
    (*task)->bf3_aes_gcm.callback_owner_thread_id = 0;
    (*task)->bf3_aes_gcm.callback_status = DPK_ONGOING;
    return true;
}
...
void *app_alloc_req_ctx_buffer(size_t size)
{
    if (size == 0)
    {
        printf("app_alloc_req_ctx_buffer got size=0\n");
        return nullptr;
    }
    ...
    void *ptr = mi_heap_malloc(this_thread_heap.heap, size);
    if (ptr == nullptr)
    {
        printf("app_alloc_req_ctx_buffer failed for size=%lu\n", (unsigned long)size);
        return nullptr;
    }
    return ptr;
}

void app_free_req_ctx_buffer(void *ptr)
{
    if (ptr == nullptr)
    {
        return;
...
struct dpm_mem_req *dpm_free_mem_buf_async(dpkernel_task *task)
{
    return _dpm_submit_shared_mem_req_async(task, DPM_MEM_REQ_FREE_IO);
}

struct dpm_mem_req *dpm_free_mem_buf_async_on_thread(dpkernel_task *task, uint32_t thread_id)
{
    return _dpm_submit_shared_mem_req_async_on_thread(task, DPM_MEM_REQ_FREE_IO, thread_id);
}

struct dpm_mem_req *dpm_alloc_task_async_on_thread(dpkernel_task *task, uint32_t thread_id)
{
    return _dpm_submit_shared_mem_req_async_on_thread(task, DPM_MEM_REQ_ALLOC_TASK, thread_id);
}

struct dpm_mem_req *dpm_free_task_async_on_thread(dpkernel_task *task, uint32_t thread_id)
{
    return _dpm_submit_shared_mem_req_async_on_thread(task, DPM_MEM_REQ_FREE_TASK, thread_id);
}
...
bool dpm_free_mem_buf(dpkernel_task *task)
{
    auto req = _dpm_submit_shared_mem_req_async(task, DPM_MEM_REQ_FREE_IO);
    if (req == NULL)
    {
        printf("mi_heap_malloc free io req allocation failed\n");
        return false;
    }
    print_debug("waiting for mem req to be done\n");
    return dpm_wait_for_mem_req_completion(req);
