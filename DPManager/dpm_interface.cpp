...
#define BATCH_SUBMIT_MAX_SIZE 1024
// struct dpm_req_msg *batch_submit_msgs;
thread_local struct dpm_req_msg batch_submit_msgs[BATCH_SUBMIT_MAX_SIZE];
...
{
#ifdef USE_RING_BUFFER
    struct dpm_req_msg *msgs;
    std::vector<struct dpm_req_msg> msg_vec;
    if (n_tasks > BATCH_SUBMIT_MAX_SIZE)
    {
        msg_vec.resize(n_tasks);
        msgs = msg_vec.data();
    }
    else
    {
        msgs = batch_submit_msgs;
    }
    ...
    while (!InsertToRequestBufferProgressive(ring_buf, (char *)msgs, sizeof(struct dpm_req_msg) * n_tasks))
        ;
#else
...
bool dpm_frontend_initialize()
{
    ...
    // alloc batch submit msgs
    // batch_submit_msgs = (struct dpm_req_msg *)calloc(BATCH_SUBMIT_MAX_SIZE, sizeof(struct dpm_req_msg));
    return true;
}
