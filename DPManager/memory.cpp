...
static std::atomic<bool> g_external_io_region_enabled{false};
static void *g_external_io_region_base = nullptr;
static size_t g_external_io_region_size = 0;

/// each dpm thread has its own heap, as required by mimalloc heaps
thread_local thread_local_mi_heap input_region_mi_heap;
/// each dpm thread has its own heap, as required by mimalloc heaps
thread_local thread_local_mi_heap output_region_mi_heap;
...
bool _setup_shared_memory(size_t shm_size, struct dpm_shared_mem *dpm_shared_mem, const char *shm_name)
{
    ...
    // maybe don't mlock the memory
    /* if (mlock(dpm_shared_mem->shm_ptr, shm_size) == -1)
    {
        perror("mlock");
        munmap(dpm_shared_mem->shm_ptr, shm_size);
        close(dpm_shared_mem->shm_fd);
        return false;
    } */

    // Eagerly pre-populate the mapping. This is intentional for predictable latency.
    if (madvise(dpm_shared_mem->shm_ptr, shm_size, MADV_POPULATE_READ | MADV_POPULATE_WRITE) == -1)
    {
        perror("madvise");
        munlock(dpm_shared_mem->shm_ptr, shm_size);
        munmap(dpm_shared_mem->shm_ptr, shm_size);
        close(dpm_shared_mem->shm_fd);
        return false;
    }
...
bool dpm_configure_external_io_region(void *io_base, size_t io_size)
{
    if (io_base == nullptr || io_size == 0)
    {
        printf("dpm_configure_external_io_region got invalid base/size base=%p size=%lu\n", io_base,
               (unsigned long)io_size);
        return false;
    }
    g_external_io_region_base = io_base;
    g_external_io_region_size = io_size;
    g_external_io_region_enabled.store(true, std::memory_order_release);
    return true;
}

bool _setup_shared_memory_regions(struct dpm_io_mem_region *io_region, struct dpm_shared_mem *req_ctx_shm)
{
    if (g_external_io_region_enabled.load(std::memory_order_acquire))
    {
        // Embedded mode: IO memory is owned by the caller (e.g., DDS SPDK DMA region).
        io_region->input_region.shm_ptr = g_external_io_region_base;
        io_region->input_region.shm_size = g_external_io_region_size;
        io_region->input_region.shm_fd = -1;
        io_region->output_region = io_region->input_region;
    ...
    // In single-IO mode, input/output intentionally alias one shared-memory backing.
    if (!_setup_shared_memory(DPM_COMBINED_IO_SHM_SIZE, &io_region->input_region, DPM_SHM_IO_REGION_NAME))
    {
        printf("setup_shared_memory combined io region failed\n");
        return false;
    }
    io_region->output_region = io_region->input_region;
#else
...
bool setup_mimalloc(struct dpm_io_mem_region *io_region, struct dpm_shared_mem *req_ctx_shm)
{
    ...
    const bool shared_io_backing = dpm_io_regions_share_backing(io_region);

    // memset the underlying memory to 0
    memset(io_region->input_region.shm_ptr, 0, io_region->input_region.shm_size);
    if (!shared_io_backing)
    {
        memset(io_region->output_region.shm_ptr, 0, io_region->output_region.shm_size);
    }
    memset(req_ctx_shm->shm_ptr, 0, req_ctx_shm->shm_size);

    if (!mi_manage_os_memory_ex(io_region->input_region.shm_ptr, io_region->input_region.shm_size, true, false, true,
                                -1, true, &io_region->input_region.arena_id))
    {
    ...
    {
        printf("input region arena id: %d\n", io_region->input_region.arena_id);
    }
    if (shared_io_backing)
    {
        // Single-IO mode: output region aliases input region, so they must share the same arena id.
        io_region->output_region.arena_id = io_region->input_region.arena_id;
        printf("output region aliases input region arena id: %d\n", io_region->output_region.arena_id);
    }
    else
    {
        if (!mi_manage_os_memory_ex(io_region->output_region.shm_ptr, io_region->output_region.shm_size, true, false, true,
                                    -1, true, &io_region->output_region.arena_id))
        {
        ...
        {
            printf("output region arena id: %d\n", io_region->output_region.arena_id);
        }
    }
    // mimalloc seems to always zero out the memory
    // memset(dpm_shared_mem->shm_ptr, 42, dpm_shared_mem->shm_size);

    // NOTE: initialization is now done the first time any thread calls allocate input/output
    ...
    /* mi_arenas_print();
    size_t arena_size = 0;
    mi_arena_area(io_region->input_region.allocator.arena_id, &arena_size);
    printf("input region arena size: %lu\n", arena_size);
    mi_arena_area(io_region->output_region.allocator.arena_id, &arena_size);
    printf("output region arena size: %lu\n", arena_size);
    void *test_ptr = mi_heap_malloc(io_region->input_region.allocator.heap, 1);
    if (test_ptr == NULL)
    {
        printf("mi_heap_malloc failed to malloc 1 for input region\n");
        // return false;
    }
    test_ptr = mi_heap_malloc(io_region->output_region.allocator.heap, 1);
    if (test_ptr == NULL)
    {
        printf("mi_heap_malloc failed to malloc 1 for output region\n");
        // return false;
    }
    printf("can allocate 1 byte\n");
    mi_arenas_print();

    test_ptr = mi_heap_malloc(io_region->input_region.allocator.heap, 1245184);
    if (test_ptr == NULL)
    {
        printf("mi_heap_malloc failed to malloc 1245184 for input region\n");
        // return false;
    }
    test_ptr = mi_heap_malloc(io_region->output_region.allocator.heap, 1245184);
    if (test_ptr == NULL)
    {
        printf("mi_heap_malloc failed to malloc 1245184 for output region\n");
        // return false;
    }
    printf("can allocate 1245184 bytes\n");
    mi_arenas_print(); */

    // see which size would cause it to fail
    // bool to_break = false;
    // // bool bad_start = false;
    // for (size_t i = 1048576; i < 128 * 1024 * 1024; i++)
    // {
    //     void *test_ptr = mi_heap_malloc(io_region->input_region.allocator.heap, i);
    //     if (test_ptr == NULL)
    //     {
    //         printf("mi_heap_malloc failed to malloc %lu for input region\n", i);
    //         // break;
    //         to_break = true;
    //         // bad_start = true;
    //     }
    //     mi_free(test_ptr);
    //     test_ptr = mi_heap_malloc(io_region->output_region.allocator.heap, i);
    //     if (test_ptr == NULL)
    //     {
    //         printf("mi_heap_malloc failed to malloc %lu for output region\n", i);
    //         // break;
    //         to_break = true;
    //         // bad_start = true;
    //     }
    //     mi_free(test_ptr);
    //     // test_ptr = mi_heap_malloc(req_ctx_shm->allocator.heap, i);
    //     // if (test_ptr == NULL)
    //     // {
    //     //     printf("mi_heap_malloc failed to malloc %lu for req_ctx region\n", i);
    //     //     // break;
    //     //     to_break = true;
    //     // }
    //     // mi_free(test_ptr);
    //     // test_ptr = mi_heap_malloc(mi_heap_get_default(), i);
    //     // if (test_ptr == NULL)
    //     // {
    //     //     printf("mi_heap_malloc failed to malloc %lu for default heap\n", i);
    //     //     // break;
    //     //     to_break = true;
    //     // }
    //     // mi_free(test_ptr);
    ...
    //     else
    //     {
    //         // printf("can allocate %lu bytes\n", i);
    //     }
    // }

    // printf("heap get backing = %p\n", mi_heap_get_backing());
    // printf("heap get default = %p\n", mi_heap_get_default());
    // printf("req_ctx_shm->allocator.heap = %p\n", req_ctx_shm->allocator.heap);
    // printf("io_region->output_region.allocator.heap = %p\n", io_region->output_region.allocator.heap);
    // printf("io_region->input_region.allocator.heap = %p\n", io_region->input_region.allocator.heap);
...
{
    if (dpm_shared_mem->shm_fd < 0)
    {
        // External memory regions are caller-owned and should not be unmapped/unlinked by DPM.
        return true;
    }
...
void *dpm_get_req_ctx_ptr_from_shmptr(shm_ptr offset)
{
    return (void *)((char *)dpm_req_ctx_shm.shm_ptr + offset);
}

dpkernel_task *dpm_get_task_ptr_from_shmptr(shm_ptr offset)
{
    return (dpkernel_task *)((char *)dpm_req_ctx_shm.shm_ptr + offset);
}

shm_ptr dpm_get_shmptr_for_task_ptr(dpkernel_task *task)
{
    return shm_ptr((char *)task - (char *)dpm_req_ctx_shm.shm_ptr);
}
...
{
    if (input_region_mi_heap.heap == NULL)
    {
        // this thread has not initialized the heap, so we need to do it
        auto heap = mi_heap_new_in_arena(dpm_own_mem_region.input_region.arena_id);
        if (heap == NULL)
        {
            printf("mi_heap_new_in_arena failed for input regionm thread id = %lu\n", pthread_self());
            return nullptr;
        }
        input_region_mi_heap.heap = heap;
    }
    return (char *)mi_heap_malloc(input_region_mi_heap.heap, size);
}

char *allocate_output_buf(size_t size)
{
    if (output_region_mi_heap.heap == NULL)
    {
        // this thread has not initialized the heap, so we need to do it
        auto heap = mi_heap_new_in_arena(dpm_own_mem_region.output_region.arena_id);
        if (heap == NULL)
        {
            printf("mi_heap_new_in_arena failed for output regionm thread id = %lu\n", pthread_self());
            return nullptr;
        }
        output_region_mi_heap.heap = heap;
    }
    return (char *)mi_heap_malloc(output_region_mi_heap.heap, size);
}
