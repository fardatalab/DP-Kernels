...
/* bool _setup_shm_region_exchange_queue_dpm()
{
    dpm_shm_region_exchange_queue =
        new boost::interprocess::message_queue(boost::interprocess::create_only, DPM_MEM_REGION_EXCHANGE_QUEUE_NAME,
                                               DPM_MEM_REGION_EXCHANGE_QUEUE_SIZE, sizeof(struct
dpm_io_mem_region)); return true;
} */
...
bool setup_doca_shared_mem(const char *pci_addr, struct dpm_doca_state *dpm_doca_state)
{
    ...
    // XXX: 2.5 has breaking changes, fix this later
    result = doca_compress_create(dpm_doca_state->state.dev, &dpm_doca_state->compress);
    if (result != DOCA_SUCCESS)
    {
        printf("Unable to create compress engine: %s\n", doca_error_get_descr(result));
        // return false;
    }
    dpm_doca_state->state.ctx = doca_compress_as_ctx(dpm_doca_state->compress);
    if (dpm_doca_state->state.ctx == NULL)
    {
        printf("Unable to get ctx from compress engine: %s\n", doca_error_get_descr(result));
        // return false;
    }

    // XXX: no need for separate mem and workq init?
