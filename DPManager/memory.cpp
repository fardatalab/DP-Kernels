...
{
    dpm_shm_region_exchange_queue =
        new boost::interprocess::message_queue(boost::interprocess::create_only, DPM_MEM_REGION_EXCHANGE_QUEUE_NAME,
                                               DPM_MEM_REGION_EXCHANGE_QUEUE_SIZE, sizeof(struct dpm_io_mem_region));
    return true;
} */
...
bool setup_bluefield(const char *pci_addr, struct dpm_doca_state *dpm_doca_state)
{
    ...
    // NOTE: need to setup ctx first before setup_doca_shared_mem
    doca_error_t result = doca_compress_create(&dpm_doca_state->compress);
    if (result != DOCA_SUCCESS)
    {
        printf("Unable to create compress engine: %s", doca_get_error_string(result));
        return false;
    }
    dpm_doca_state->state.ctx = doca_compress_as_ctx(dpm_doca_state->compress);
    ...
    ret = setup_doca_shared_mem(pci_addr, dpm_doca_state);
    if (!ret)
    {
    ...
    ret = init_core_objects_workq(&dpm_doca_state->state, DPM_DOCA_WORKQ_DEPTH);
    if (ret != DOCA_SUCCESS)
    {
...
// TODO: later will need to do doca_compress_create to create the ctx
// and init_core_objects_workq to create the workq
bool setup_doca_shared_mem(const char *pci_addr, struct dpm_doca_state *dpm_doca_state)
{
    ...
    if (doca_mmap_set_memrange(dpm_doca_state->state.src_mmap, dpm_own_mem_region.input_region.shm_ptr,
                               dpm_own_mem_region.input_region.shm_size) != DOCA_SUCCESS)
    {
        printf("doca_mmap_set_memrange failed for src_mmap\n");
        return false;
    }
    printf("doca_mmap_set_memrange succeeded for src_mmap\n");
    if (doca_mmap_set_memrange(dpm_doca_state->state.dst_mmap, dpm_own_mem_region.output_region.shm_ptr,
                               dpm_own_mem_region.output_region.shm_size) != DOCA_SUCCESS)
    {
...
bool _setup_mem_req_queue()
{
    ...
    dpm_mem_req_queue = new boost::interprocess::message_queue(boost::interprocess::create_only, DPM_MEM_REQ_QUEUE_NAME,
...
bool dpm_poll_mem_req()
{
    ...
    unsigned int priority;
    shm_ptr req_ptr;
    if (dpm_mem_req_queue->try_receive(&req_ptr, sizeof(req_ptr), recvd_size, priority))
    {
...
bool dpm_mem_init()
{
    ...
    /* _dpm_send_mem_region_info();
    _dpm_recv_mem_region_info() */
}
