...
/// Embedded-mode frontend initialization.
/// Reuses manager-owned memory regions in-process instead of creating new shm IO regions.
...
/// Resolve an arbitrary req_ctx shared-memory offset into a local pointer in app process.
inline void *app_get_req_ctx_ptr_from_shmptr(shm_ptr offset)
{
    return (void *)((char *)app_req_ctx_shm.shm_ptr + offset);
}
/// Convert a req_ctx local pointer to shared-memory offset (for control-plane metadata such as key bytes).
inline shm_ptr app_get_shm_ptr_for_req_ctx_ptr(void *ptr)
{
    return shm_ptr((char *)ptr - (char *)app_req_ctx_shm.shm_ptr);
}
...
/// Allocate both input/output buffers through one memory request roundtrip.
...
/// Allocate arbitrary bytes from app req_ctx shared memory.
/// This keeps allocator internals hidden from client code while ensuring returned pointers
/// are convertible with app_get_shm_ptr_for_req_ctx_ptr().
void *app_alloc_req_ctx_buffer(size_t size);

/// Free a pointer previously returned by app_alloc_req_ctx_buffer().
...
/// Allocate/free kernel-internal task objects (e.g., preallocated DOCA task handles).
...
/// Enable/disable lazy DOCA data binding on execute path for this task.
inline void dpm_set_task_lazy_bind_mode(dpkernel_task *task, bool enable)
{
    if (task == nullptr)
    {
        return;
    }
    task->base.use_lazy_doca_buf_binding = enable;
}
