...
/// Configure an externally-managed contiguous IO region for DPM input/output.
/// Must be called before dp_kernel_manager_msgq_start().
extern bool dpm_configure_external_io_region(void *io_base, size_t io_size);
...
/// Return true when input/output regions alias the same shared-memory backing.
inline bool dpm_io_regions_share_backing(const struct dpm_io_mem_region *io_region)
{
    if (io_region == nullptr)
    {
        return false;
    }
    return io_region->input_region.shm_ptr == io_region->output_region.shm_ptr &&
           io_region->input_region.shm_fd == io_region->output_region.shm_fd &&
           io_region->input_region.shm_size == io_region->output_region.shm_size;
}
...
// NOTE (legacy): device-specific DOCA setup/cleanup APIs used to be declared here:
// bool setup_bluefield(const char *pci_addr, struct dpm_doca_state *dpm_doca_state);
// bool cleanup_bluefield();
// They are intentionally not part of the common memory interface anymore.
...
// NOTE (legacy): common DOCA shared-memory setup API removed from this header:
...
/// Resolve an arbitrary req_ctx shared-memory offset into a local pointer.
