...
#define DPM_REQ_CTX_SHM_SIZE 128 * 1024 * 1024 // 128MB
...
// put into interface's declaration
// app uses this
// extern struct dpm_io_mem_region dpm_mem_region;

struct dpm_mem_region_exchange
{
    struct dpm_io_mem_region *app_mem_region;
    struct dpm_io_mem_region *dpm_mem_region;
    /* std::atomic<int> dpm_start;
    std::atomic<int> dpm_done;
    std::atomic<int> app_start;
    std::atomic<int> app_done; */
};

/// DPM needs to know where the application has placed the input and output buffers
bool _exchange_mem_region_info_old(struct dpm_io_mem_region *dpm_mem_region, struct dpm_mem_region_exchange *exchange);
...
struct dpm_mem_req
{
    ...
    std::atomic_bool done; // set to true when the request is done, app should poll this for completion
};

// boost message queue for memory requests
extern boost::interprocess::message_queue *dpm_mem_req_queue;
#define DPM_MEM_REQ_QUEUE_NAME "dpm_mem_req_queue"
#define DPM_MEM_REQ_QUEUE_SIZE 4096
