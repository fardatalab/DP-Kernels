...
std::string INPUT_FILE = "/home/ubuntu/deflate/1M.deflate";
std::string OUTPUT_FILE = "/home/ubuntu/txt/1M.txt";
int OUTPUT_SIZE = 1024 * KB;
...
void worker_thread(int thread_id, int num_threads, char *input_buf, size_t input_file_size)
{
    ...
    for (int i = 0; i < N_BUFS; ++i)
    {
        ...
        temp_task->device = dpm_device::DEVICE_SOFTWARE;
    ...
    for (int i = 0; i < DEPTH; ++i)
    {
        ...
        do
        {
            ...
            dpm_submit_task_msgq_blocking(thread_id, dpk_task,
                                          dpm_device::DEVICE_SOFTWARE); // each thread has its own queue
...
int main(int argc, char *argv[])
{
    ...
    printf("throughput (MB/s): %.6f\n", (double)total_completed.load() * OUTPUT_SIZE / elapsed_time / 1024 / 1024);
