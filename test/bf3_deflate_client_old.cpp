...
#define INPUT_FILE "/home/ubuntu/deflate/1M.deflate"
...
void worker_thread(int thread_id, int num_threads)
{
...
#endif
    dpkernel_task *task;
    dpkernel_task_base *actual_task;
    bool ret;
    ...
    for (int i = 0; i < DEPTH; ++i)
    {
        ...
        ret = dpm_alloc_input_buf(temp_task->in_size, alloc_task);
        if (!ret)
        {
        ...
        ret = dpm_alloc_output_buf(OUTPUT_SIZE, alloc_task);
        if (!ret)
        {
    ...
    for (int i = 0; i < DEPTH; ++i)
    {
        ...
        do
        {
            ...
            // this is every call doing round robin on ring buffers
            // dpm_submit_task_msgq_multithread(task, dpm_device::DEVICE_BLUEFIELD_3);

            // this is each thread with its own ring buffer
    ...
    actual_task = &task->base;
    while (actual_task->completion.load(std::memory_order_acquire) != DPK_SUCCESS)
    {
        std::this_thread::yield();
    }
    ...
    for (auto task : task_list)
    {
        ...
        char *in = app_get_input_ptr_from_shmptr(dpk_task.in);
        char *out = app_get_output_ptr_from_shmptr(dpk_task.out);
