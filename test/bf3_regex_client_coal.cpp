...
//// NOTE: client for COALESCING enabled, will make sure the inputs are contiguous and coalescable

std::string INPUT_FILE = "/home/ubuntu/regex_files/lineitem256M.tbl.old";

int INPUT_SIZE = 1024; // max size per batch
...
void worker_thread(int thread_id, int num_threads, shm_ptr large_chunk_in_buf, shm_ptr large_chunk_out_buf, int n_bufs,
                   int *buf_sizes)
{
#ifdef PIN_THREAD
    // Pin thread to core
    ...
    int n_bufs_per_thread = n_bufs / num_threads;
    ...
    for (int i = 0; i < n_bufs_per_thread; ++i)
    {
        ...
        ////
        temp_task->in = large_chunk_in_buf + (i + thread_id * n_bufs_per_thread) * INPUT_SIZE;
        temp_task->out = large_chunk_out_buf + (i + thread_id * n_bufs_per_thread) * OUTPUT_SIZE;
        ////

        auto in_req = dpm_alloc_input_buf_async(INPUT_SIZE, alloc_task);
        if (in_req == NULL)
        {
    ...
    for (int i = 0; i < n_bufs_per_thread; ++i)
    {
        ...
        in_buf = temp_task->in;
        // memcpy(app_get_input_ptr_from_shmptr(temp_task->in), input_string_bufs[thread_id * num_threads + i],
        //        buf_sizes[thread_id * num_threads + i]);
    ...
    for (int i = 0; i < n_bufs_per_thread; i += REQ_BATCH_SIZE)
    {
        ...
        do
        {
            ...
            int n_tasks = n_bufs_per_thread - i < REQ_BATCH_SIZE ? n_bufs_per_thread - i : REQ_BATCH_SIZE;
            dpm_submit_task_msgq_batched(thread_id, &task_list[i], n_tasks, DEVICE_BLUEFIELD_3);
    ...
    auto dpk_task = task_list[n_bufs_per_thread - 1];
    task = &dpk_task->base;
    ...
    for (int i = 0; i < n_bufs_per_thread - 1; ++i)
    {
        auto dpk_task = task_list[i];
        task = &dpk_task->base;
        while (task->completion.load(std::memory_order_release) != DPK_SUCCESS)
    ...
    completed_cnt += n_bufs_per_thread;
    ...
    for (int i = 0; i < n_bufs_per_thread; i++)
    {
        ...
        // this would make sure mimalloc won't free the memory, since it doesn't come from mimalloc (not individually
        // allocated)
...
int main(int argc, char *argv[])
{
    ...
    // allocate large chunk of input and output buffers
    ...
    auto alloc_req = dpm_alloc_input_buf_async(N_BUFS * INPUT_SIZE, large_chunk_task);
    dpm_wait_for_mem_req_completion(alloc_req);
    alloc_req = dpm_alloc_output_buf_async(N_BUFS * OUTPUT_SIZE, large_chunk_task);
    dpm_wait_for_mem_req_completion(alloc_req);

    // char *large_chunk_in_buf = app_get_input_ptr_from_shmptr(large_chunk_task->base.in);
    // char *large_chunk_out_buf = app_get_output_ptr_from_shmptr(large_chunk_task->base.out);

    shm_ptr large_chunk_in_buf = large_chunk_task->base.in;
    shm_ptr large_chunk_out_buf = large_chunk_task->base.out;

    printf("Allocated large chunk for input and output buffer\n");
    memcpy(app_get_input_ptr_from_shmptr(large_chunk_in_buf), input_buf, file_size);
    ...
    {
        threads.emplace_back(worker_thread, i, NUM_THREADS, large_chunk_in_buf, large_chunk_out_buf, N_BUFS, buf_sizes);
    ...
    printf("throughput (MB/s): %.6f\n", total_completed.load() * INPUT_SIZE / elapsed_time / 1024 / 1024);

    // free the large chunk task
