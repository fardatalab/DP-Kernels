...
std::vector<std::chrono::duration<double>> per_thread_elapsed;
...
void worker_func(size_t tid, size_t bid, size_t start_buf, size_t end_buf, size_t* result) {
	...
	*result = num_matches;
	...
	per_thread_elapsed[tid] += (end - start);
...
int main(int argc, char** argv) {
    ...
    if (argc > 2) pipeline_depth = atoi(argv[2]);

    input_buf_alloc_reqs.resize(n_threads);
    output_buf_alloc_reqs.resize(n_threads);
    input_buf_dealloc_reqs.resize(n_threads);
    output_buf_dealloc_reqs.resize(n_threads);
    per_thread_task_lists.resize(n_threads);
	per_thread_elapsed.resize(n_threads);
    ...
    for (size_t b = 0; b < n_batches; b++) {
        ...
        size_t workload = batch_size / n_threads;
        for (size_t i = 0; i < n_threads; i++) {
            size_t start = start_buf + i * workload;
            size_t end = (i == n_threads-1) ? end_buf : start_buf + (i+1) * workload;
	...
	for (size_t i = 0; i < n_threads; i++) {
		if( per_thread_elapsed[i].count() > latency_sec) latency_sec = per_thread_elapsed[i].count();
	}
...
void dpm_deallocate_bufs(size_t tid, size_t bid, size_t n_bufs) {
    ...
    for (size_t i = 0; i < n_bufs; i++) {
		...
		app_free_task_request(per_thread_task_lists[tid][i]);
	}
}
