...
bool PageServer::InitializeIndex(uint32_t num_pages)
{
  ...
  for (size_t i = 0; i < num_replications; ++i) {
    // open the file this many times. Note that this is not necessary, we can just open it once and just compute offset
...
void PageServer::AcceptorThread()
{
    ...
    while (running_)
    {
        ...
        // Create client state
        {
            ...
            client_states_[client_fd]->scratch = new char[VALUE_SIZE * 2]; // buffer
        }
