...
class queue
{
  ...
  public:
    ...
    pool_t pool;
    /// get the pointer to the underlying memory, and fill in the number of bytes
    char *get_pool(size_t *bytes)
    {
        // boost::array<char, 1024> pool;
        // *bytes = pool.data.size();
        size_t bytes = 0;
        char *p = pool.get_shm_data(bytes);
        printf("pool size: %lu\n", *bytes);
        // return pool.data.c_array();
    }
     ...
     * */
    queue(void) : head_(tagged_node_handle(0, 0)), tail_(tagged_node_handle(0, 0)), pool(node_allocator(), capacity)
    {
    ...
    explicit queue(typename boost::allocator_rebind<node_allocator, U>::type const &alloc)
        : head_(tagged_node_handle(0, 0)), tail_(tagged_node_handle(0, 0)), pool(alloc, capacity)
    {
    ...
    explicit queue(size_type n)
        : head_(tagged_node_handle(0, 0)), tail_(tagged_node_handle(0, 0)), pool(node_allocator(), n + 1)
    {
        // Don't use BOOST_STATIC_ASSERT() here since it will be evaluated when compiling
        // this function and this function may be compiled even when it isn't being used.
        BOOST_ASSERT(!has_capacity);
    ...
    queue(size_type n, typename boost::allocator_rebind<node_allocator, U>::type const &alloc)
        : head_(tagged_node_handle(0, 0)), tail_(tagged_node_handle(0, 0)), pool(alloc, n + 1)
    {
        BOOST_STATIC_ASSERT(!has_capacity);
        initialize();
    }
  ...
  private:
    ...
    // make public to use in shared memory
    // pool_t pool;
#endif
};
