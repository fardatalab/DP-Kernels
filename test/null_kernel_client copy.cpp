...
void worker_thread(int thread_id, int num_threads)
{
    ...
    for (int i = 0; i < DEPTH; i += BATCH_SIZE)
    {
        ...
        do
        {
            ...
            int n_tasks = DEPTH - i < BATCH_SIZE ? DEPTH - i : BATCH_SIZE;
            dpm_submit_task_msgq_batched(thread_id, &task_list[i], n_tasks, dpm_device::DEVICE_NULL);
#else
