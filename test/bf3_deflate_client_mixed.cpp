...
#define SMALL_REQ_WEIGHT 0.0
#define MED_REQ_WEIGHT 0.0
#define LARGE_REQ_WEIGHT 1.0
...
int NUM_THREADS = 2; // default to 1 thread
...
void worker_thread(int thread_id, int num_threads, char *input_buf, size_t input_file_size)
{
    ...
    const int total_tasks = static_cast<int>(task_list.size());
    if (DEPTH != total_tasks)
    {
        // DEPTH is currently an input arg, but this worker generates/submits task_list.size() tasks.
        // Keep behavior explicit to avoid freeing only a prefix of submitted tasks.
        printf("Thread %d: DEPTH (%d) != total_tasks (%d); using total_tasks for completion/free loops\n", thread_id, DEPTH,
               total_tasks);
    }
    ...
    for (int i = 0; i < total_tasks; i += REQ_BATCH_SIZE)
    // for (int i = 0; i < N_BUFS; i++)
    {
        ...
        do
        {
            ...
            int n_tasks = total_tasks - i < REQ_BATCH_SIZE ? total_tasks - i : REQ_BATCH_SIZE;
            // #ifdef CHECK_LATENCY
    ...
    retry_tasks.reserve(total_tasks);
    completed_tasks.reserve(total_tasks);
    uint completed_task_cnt = 0, retry_task_cnt = 0; //, ongoing_task_cnt = total_tasks;
    ...
    while (completed_task_cnt < (uint)total_tasks || retry_task_cnt > 0)
    {
    ...
    for (int i = 0; i < total_tasks; i++)
    {
        if (!dpm_wait_for_mem_req_completion(free_in_bufs_req[i]))
        {
            printf("Thread %d: input free mem req %d failed\n", thread_id, i);
        }
        if (!dpm_wait_for_mem_req_completion(free_out_bufs_req[i]))
        {
            printf("Thread %d: output free mem req %d failed\n", thread_id, i);
        }
    }
