...
void worker_thread(int thread_id, int num_threads, char **input_string_bufs, int n_bufs, int *buf_sizes)
{
    ...
    for (int i = 0; i < N_BUFS / num_threads; ++i)
    {
        ...
        temp_task->in_size = buf_sizes[thread_id * num_threads + i];
        ...
        auto in_req = dpm_alloc_input_buf_async(buf_sizes[thread_id * num_threads + i], alloc_task);
        if (in_req == NULL)
        {
    ...
    for (int i = 0; i < DEPTH / num_threads; ++i)
    {
    ...
    auto dpk_task = task_list[DEPTH / num_threads - 1];
    task = &dpk_task->base;
    ...
    for (int i = 0; i < DEPTH / num_threads - 1; ++i)
    {
        auto dpk_task = task_list[i];
        task = &dpk_task->base;
        while (task->completion.load(std::memory_order_release) != DPK_SUCCESS)
    ...
    completed_cnt += DEPTH / num_threads;
...
int main(int argc, char *argv[])
{
    ...
    printf("throughput (MB/s): %.6f\n", N_BUFS * INPUT_SIZE / elapsed_time / 1024 / 1024);
