...
template <typename T, typename Alloc = std::allocator<T>> class freelist_stack : Alloc
{
  ...
  public:
    ...
    freelist_stack(Allocator const &alloc, std::size_t n = 0) : Alloc(alloc), pool_(tagged_node_ptr(NULL))
    {
    ...
    template <bool ThreadSafe, bool Bounded, typename ArgumentType> T *construct(ArgumentType const &arg)
    {
        T *node = allocate<ThreadSafe, Bounded>();
        if (node)
            new (node) T(arg);
    ...
    template <bool ThreadSafe> void destruct(tagged_node_handle const &tagged_ptr)
    {
...
template <typename T, std::size_t size> struct compiletime_sized_freelist_storage
{
  ...
  public:
    ...
    boost::array<char, size * sizeof(T) + 64> *data = new boost::array<char, size * sizeof(T) + 64>;

    char *get_shm_data(size_t *bytes)
    {
        *bytes = data->size();
        return data->c_array();
    }
        ...
        : allocator_type(alloc), node_count_(count)
    {
        if (count > 65535)
            boost::throw_exception(
                std::runtime_error("boost.lockfree: freelist size is limited to a maximum of 65535 objects"));
...
template <typename T, typename NodeStorage = runtime_sized_freelist_storage<T>> class fixed_size_freelist : NodeStorage
{
    struct freelist_node
    {
        tagged_index next;
  ...
  public:
    ...
    fixed_size_freelist(Allocator const &alloc, std::size_t count)
        : NodeStorage(alloc, count), pool_(tagged_index(static_cast<index_t>(count), 0))
    {
        initialize();
    }
...
{
    typedef typename mpl::if_c<IsCompileTimeSized, compiletime_sized_freelist_storage<T, Capacity>,
                               runtime_sized_freelist_storage<T, Alloc>>::type fixed_sized_storage_type;

    typedef typename mpl::if_c<IsCompileTimeSized || IsFixedSize, fixed_size_freelist<T, fixed_sized_storage_type>,
