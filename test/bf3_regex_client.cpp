...
int main(int argc, char *argv[])
{
    ...
    size_t string_size = load_strings_in_batch(string_buf, INPUT_SIZE);
    while (string_size > 0)
    {
        parsed_strings.push_back(string_buf);
        string_buf_sizes.push_back(string_size);
        N_BUFS++;

        string_buf = new char[INPUT_SIZE];
        string_size = load_strings_in_batch(string_buf, INPUT_SIZE);
    }
    printf("duckdb parser produced N_BUFS: %d with chunk size = %d\n", N_BUFS, INPUT_SIZE);
    // for (int i = 0; i < N_BUFS; i++)
    // {
